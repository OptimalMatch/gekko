...
//  https://bitcointalk.org/index.php?topic=290961.0
config.DEMA = {
  // EMA weight (α)
...
if(config.watch.exchange == 'GDAX') {
  config.trader = {
    enabled: true,
    key: Prop.getPrivateCode('GDAX').key,
    secret: Prop.getPrivateCode('GDAX').secret,
    username: Prop.getPrivateCode('GDAX').username, // your username, only required for specific exchanges.
    passphrase: Prop.getPrivateCode('GDAX').passphrase // GDAX, requires a passphrase.
  }
} else if(config.watch.exchange == 'Bitfinex') {
  config.trader = {
    enabled: true,
    key: Prop.getPrivateCode('Bitfinex').key,
    secret: Prop.getPrivateCode('Bitfinex').secret,
    username: Prop.getPrivateCode('Bitfinex').username, // your username, only required for specific exchanges.
    passphrase: Prop.getPrivateCode('Bitfinex').passphrase // GDAX, requires a passphrase.
  }
}
...
config.mailer = {
  ...
  email: 'alex@optimalmatch.com',    // Your Gmail address
  muteSoft: true, // disable advice printout if it's soft
  ...
  password: 'vlktrfoexqhvjkrn',       // Your Gmail Password - if not supplied Gekko will prompt on startup.

  tag: '[GEKKO-GDAX] ',      // Prefix all email subject lines with this
  ...
  user: 'alex@optimalmatch.com',       // Your Email server user name - usually your full Email address 'me@mydomain.com'
  from: 'alex@optimalmatch.com',       // 'me@mydomain.com'
  to: 'alex.chang@unidatum.com',       // 'me@somedomain.com, me@someotherdomain.com'
